...
int scale_chromo_1[13] = {NOTE_C4, NOTE_CS4, NOTE_D4, NOTE_DS4, NOTE_E4, NOTE_F4, NOTE_FS4, NOTE_G4, NOTE_GS4, NOTE_A4, NOTE_AS4, NOTE_B4, NOTE_C3};

int scale_diatonic_1[8] = {NOTE_C4, NOTE_D4, NOTE_E4, NOTE_F4, NOTE_G4, NOTE_A4, NOTE_B4, NOTE_C3};
...
//      Serial.println(minPot);
    ...
    if ((maxLight - minLight) > rangeTune)
    {
      ...
      noteMax = 8; // diatonic
      //noteMax = 6 // penta
      int note = map(potPos, minLight, maxLight, 0, noteMax);
     
      // now display light and play pitch
      Pot2Led(thisLight);
//      tone(spk, thisPitch);
//    tone(spk, scale_chromo_1[note]); 
      tone(spk, scale_diatonic_1[note]); 
    } 
    else
